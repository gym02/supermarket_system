          ...
          <el-row :gutter="20">
            ...
            <el-col :span="6">
              <el-form-item label="会员邮箱">
                <el-input v-model="searchPerson.hyEmail"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-row :gutter="20">
            <el-col :span="6">
              <el-form-item label="会员余额">
                <el-input v-model="searchPerson.hyBalance"></el-input>
              </el-form-item>
            </el-col>

            <el-col :span="6">
              <el-form-item label="会员积分">
                <el-input v-model="searchPerson.hyPoint"></el-input>
              </el-form-item>
            </el-col>

            <el-col :span="6">
              <el-form-item label="总积分">
                <el-input v-model="searchPerson.hyTotalPoint"></el-input>
              </el-form-item>
            </el-col>
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-tag
                :type="
                  scope.row.hyLevel === 5 || scope.row.hyLevel === 4
                    ? 'primary'
                    : 'success'
                "
                disable-transitions
                >{{ scope.row.hyLevel }}</el-tag
              >
            </template>
          </el-table-column>
          ...
          <el-table-column
            ...
            sortable
          >
            <template slot-scope="scope">{{ scope.row.hyDiscount }}</template>
          </el-table-column>
          ...
          <el-table-column
            ...
            sortable
          >
            <template slot-scope="scope">{{ scope.row.hyBalance }}</template>
          </el-table-column>
          ...
          <el-table-column
            ...
            sortable
          >
            <template slot-scope="scope">{{ scope.row.hyPoint }}</template>
          </el-table-column>
          ...
          <el-table-column
            ...
            sortable
          >
            <template slot-scope="scope">{{ scope.row.hyTotalPoint }}</template>
          </el-table-column>
          ...
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
              ...
              <button
                size="small"
                type="button"
                @click="findPassword(scope.$index, scope.row)"
                class="css-button-rounded--red"
              >
                <i class="el-icon-coin"></i>&nbsp;找回密码
              </button>
  ...
  methods: {
    ...
    remove(index, row) {
      this.$confirm("此操作将删除信息, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$axios
            .get("http://127.0.0.1:8080/vip/removeVip/" + row.hyId)
            .then((resp) => {
    ...
    search() {
      this.$axios
        .post("http://127.0.0.1:8080/vip/searchVip", this.searchPerson)
        .then((resp) => {
          if (resp.data.code == 200) {
    ...
    // 跳转找回密码界面
    findPassword(index,row){
      this.$router.push({name:"vipFindPassword",params:{hyName:row.hyName,gkEmail:row.gkEmail}})
    },
    // 导出excel
    exportVip() {
      let url = "http://localhost:8080/vip/export";
      url += "?pageSize=" + this.searchPerson.pageSize;
      url += "&pageNum=" + this.searchPerson.pageNum;
      if (this.searchPerson.hyId) {
        url += "&hyId=" + this.searchPerson.hyId;
      }
      if (this.searchPerson.hyName) {
        url += "&hyName=" + this.searchPerson.hyName;
      }
      if (this.searchPerson.gkEmail) {
        url += "&gkEmail=" + this.searchPerson.gkEmail;
      }
      if (this.searchPerson.hyBalance) {
        url += "&hyBalance=" + this.searchPerson.hyBalance;
      }
      if (this.searchPerson.hyPoint) {
        url += "&hyPoint=" + this.searchPerson.hyPoint;
      }
      if (this.searchPerson.hyTotalPoint) {
        url += "&hyTotalPoint=" + this.searchPerson.hyTotalPoint;
      }
