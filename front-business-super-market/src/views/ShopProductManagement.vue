          ...
          <el-table-column label="操作" fixed="right" align="center">
            <template slot-scope="scope">
              <button
                ...
                plain
                type="warning"
                @click="pullProduct(scope.$index, scope.row)"
                class="css-button-rounded--yellow"
              >
                <i class="el-icon-edit"></i>&nbsp;下架
              </button>
            </template>
          </el-table-column>
        </el-table>
    ...
    pullProduct(index, row) {
       this.$confirm("此操作将下架商品, 是否继续?", "提示", {
        ...
        .then(() => {
          console.log(row)
          this.$axios
            .post("http://localhost:8080/shopProduct/pullProductBySpId/" + row.spId+"/"+row.mdId)
            .then(r => {
              if (r.data.code === 200) {
