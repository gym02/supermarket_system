  ...
  <div>
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="供货商">
              <el-input v-model="supplyDTO.suName"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="联系人">
              <el-input v-model="supplyDTO.suContactor"></el-input>
            </el-form-item>
          </el-col>
         <el-col :span="6">
            <el-form-item label="联系电话">
              <el-input v-model="supplyDTO.suPhone"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <div>
      ...
      <el-table
        ...
        tooltip-effect="dark"
        style="width: 100%"
        :default-sort="{ supply: 'descending' }"
        :header-cell-style="{
          background: '#F5F7FA',
          'text-align': 'center',
          color: '#606266',
    ...
    <el-dialog title="show" :visible.sync="dialogVisible">
      <el-form ref="form" :model="supply" label-width="80px">
          <el-col :span="12">
            <el-form-item label="供货商">
              <el-input v-model="supply.suName"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="联系人">
              <el-input v-model="supply.suContactor"></el-input>
            </el-form-item>
          </el-col>
           <el-col :span="12">
            <el-form-item label="联系电话">
              <el-input v-model="supply.suPhone"></el-input>
            </el-form-item>
          </el-col>
      </el-form>
  ...
  methods: {
    ...
    addOrUpdate() {
      if (this.addFlag == true) {
        this.$axios
          .post("http://localhost:8080/supply/add", this.supply)
          .then(r => {
            if (r.data.code === 200) {
        ...
        // 修改
        this.$axios
          .post("http://localhost:8080/supply/edit", this.supply)
          .then(r => {
            if (r.data.code == 200) {
    ...
    remove(index, row) {
      this.$confirm("此操作将删除信息, 是否继续?", "提示", {
        ...
        type: "warning"
          ...
          .then(() => {
            this.$axios
          .post("http://localhost:8080/supply/deleteBySuId/"+row.suId)
          .then(r=>{
            if(r.data.code===200){
      ...
      this.$confirm('此操作将批量删除, 是否继续?', '提示', {
        ...
        type: 'warning'
         ...
         let suIds=[]
          ...
          this.$axios.post("http://localhost:8080/supply/removeAll",{
            suIds
          }).then(r=>{
            if(r.data.code===200){
              this.$message.success("删除成功")
    ...
    // 搜索人员
    search() {
      this.$axios.post("http://localhost:8080/supply/getAll",this.supplyDTO)
      .then(r=>{
        if(r.data.code===200){
          this.supplys=r.data.data;
        }
      })
    },
    // 搜索重置
